...
;EJ 12
(writeln "-------------------EJ 12-------------------")
(define (is_aritmetic l)
  (if (> 3 (length l))
      #t
      (and (= (- (first l) (second l))(- (second l) (third l))) (is_aritmetic (cdr l)))))
(define (is_geometric l)
  (if (> 3 (length l))
      #t
      (and (= (/ (first l) (second l))(/ (second l) (third l))) (is_geometric (cdr l)))))
(define (is_progresion l)
  (if (is_aritmetic l)
      (if (is_geometric l)
      ...
      (if (is_geometric l)
          'geometrica
          "ni geometrica ni aritmetica")))

;EJ 13
(writeln "-------------------EJ 13-------------------")
(define (random_in l) (if (= 0 (length l)) 0 (random (length l))))
(define (delete_element l n)
  (if (= n 0)
      (cdr l)
      (cons (car l) (delete_element (cdr l) (- n 1)))))
(define (_distribuir monitores jugadores index_jugador index_monitor save)
  (if (null? jugadores)
      '()
      (append save (cons (list (list-ref monitores index_monitor) (list-ref jugadores index_jugador))
...
(define (insertar_siguientes_profundidad siguientes abiertos)
  (if (null? siguientes) abiertos
      ...
      (insertar_siguientes_anchura (reverse(cdr reverse_siguientes)) (cons (car reverse_siguientes) abiertos)))))
